...
class Logger(object):
    def __init__(self, log_dir, sub_dir,use_tb=True):
        ...
        if use_tb:
            new_folder = str(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
            self._sw = SummaryWriter(str(log_dir / sub_dir / new_folder))
        else:
            self._sw = None
